...
## 最终状态（2026-09-14 晚）
- [x] M1–M8 全部完成
- [x] **49 个 GoogleTest 用例 100% 通过**（Release + ASAN/UBSan）
- [x] **90 个 pytest 接口用例全部通过**（退出码 0）
- [x] **混沌测试 5 个场景 100% 通过**（含半包/粘包、Slowloris、并发 kill）
- [x] C++ 压测客户端实测 QPS 8.2 万（同机，保守下限）
- [x] 文档齐全：API 契约 / 架构决策 / 面试准备（22 问 + 5 个踩坑故事）/ 压测记录 / 双语 README
- [ ] 可选：部署到公网服务器（需要一台 Linux 机器）

## 历史里程碑
| 阶段 | 内容 | 状态 |
|---|---|---|
| M3 | one loop per thread + 线程池 + eventfd 唤醒（目标：QPS 翻倍） | 已完成 |
